...
class SequenceGeneratorLattice(nn.Module):
   def __init__(
       self,
       models,
       tgt_dict,
   ...
   def _generate(
       ...
       if "constrained_softmax_mask" in net_input:
           constrained_softmax_mask = net_input.pop('constrained_softmax_mask')
       else:
           constrained_softmax_mask = None
       if "src_masks" in net_input:     
           src_masks = net_input['src_masks']
       else:
           src_masks = None
       ...
       for step in range(max_len + 1):  # one extra step for EOS marker
           ...
           if beam_size != 1:
               if constrained_softmax_mask is not None:
                   constrained_softmax_mask_expanded = constrained_softmax_mask.repeat(1, beam_size).view(bsz*beam_size, -1)
               else:
                   constrained_softmax_mask_expanded = None
           lprobs, avg_attn_scores = self.model.forward_decoder(
               ...
               constrained_softmax_mask=constrained_softmax_mask_expanded,
           )
           ...
           if len(finalized_sents) > 0:
               ...
               if src_masks is not None:
                   src_masks = src_masks[batch_idxs]
               if constrained_softmax_mask is not None:
                   constrained_softmax_mask = constrained_softmax_mask[batch_idxs]
   ...
   def forward_decoder(
       ...
       for i, model in enumerate(self.models):
           ...
           if self.has_incremental_states():
               ...
               decoder_out = model.decoder.forward(tokens, src_masks=src_masks, encoder_out=encoder_out,constrained_softmax_mask=constrained_softmax_mask)
...
class SequenceGeneratorWithAlignment(SequenceGeneratorLattice):
