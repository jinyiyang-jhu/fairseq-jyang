...
class TransformerLatticeEncoderLayer(nn.Module):
   """Encoder layer block.
   ...
   *args.encoder_normalize_before* to ``True``.
   ...
   def forward(self, x, encoder_padding_mask, attn_mask):
       ...
       Args:
           ...
           attn_mask (Tensor): 3D mask: it is tensor of shape `(batch_size * num_heads, tgt_len, src_len),
               it allows to specify a different mask for the entries of each batch. Here the tgt_len is
               equel to src_len
       ...
       if self.encoder_attn is not None:
           ...
           if cross_attn_mask is not None:
               if cross_attn_mask.dim() == 3:
                  if list(cross_attn_mask.size()) != [bsz * self.self_attn.num_heads, tgt_len, encoder_out.size(0)]:
                   print("shape of cross_attn_mask is {}, while it should be ({}, {}, {})".format(cross_attn_mask.shape, bsz * self.self_attn.num_heads, tgt_len, encoder_out.size(0)))
