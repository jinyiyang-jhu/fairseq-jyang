...
if [ $stage -le 0 ]; then
    ...
    bash local/lattice_preprocess/run_lattice_preprocess.sh --stage -1 \
        ...
        $bpe_code_dir $fairseq_bpe_dict_src $fairseq_bin_dir
fi
    ...
    for idx in $(seq 0 $((${#dsets[@]}-1))); do
        ...
        if [ $perform_bpe == "true" ]; then
            if [ -d $target_text_dir/$d.$target_lang ]; then
                ...
                awk 'NR==FNR {a[$1]; next} $1 in a {print $0}' $fairseq_bin_dir/$fd.uttid $input_file |\
            ...
            num_src=$(wc -l $fairseq_bin_dir/$fd.uttid | cut -d " " -f1)
