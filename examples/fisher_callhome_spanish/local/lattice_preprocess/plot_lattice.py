...
class Lattice(ReadableSentence):
   ...
   def reversed(self) -> 'Lattice':
       ...
       for node in reversed(self.nodes[:self.len_unpadded()]):
           new_node = LatticeNode(nodes_prev=[seq_len - n - 1 for n in node.nodes_next],
                                  nodes_next=[seq_len - p -
                                      1 for p in node.nodes_prev],
                                  value=node.value,
   ...
   def sent_str(self, custom_output_procs=None, **kwargs) -> str:
       ...
       """
       out_str = str([self.str_tokens(**kwargs),
                     [node.nodes_next for node in self.nodes]])
       return out_str
   ...
   def plot(self, out_file, show_log_probs=["fwd_log_prob", "marginal_log_prob", "bwd_log_prob"]):
       ...
       for i, node in enumerate(self.nodes):
           node_id = i
           log_prob_strings = [
               f"{math.exp(getattr(node,field)):.5f}" for field in show_log_probs]
   ...
   def longest_distances(self) -> List[int]:
       ...
       dist_from_start = sparse.csgraph.dijkstra(
           csgraph=adj_matrix, indices=[0])
       max_pos = int(-np.min(dist_from_start))
       dist_from_start = [int(-d) for d in dist_from_start[0]] + \
                              [max_pos] * (self.sent_len()-self.len_unpadded())
       return dist_from_start

   def compute_pairwise_log_conditionals(self, direction=None, probabilistic_masks=True):
       """
       ...
       if direction != 'bwd':
           pairwise = []
           for node_i in range(self.len_unpadded()):
               pairwise.append(self.compute_log_conditionals_one(
                   node_i, probabilistic_masks))
           pairwise_fwd = np.asarray(pairwise)

       if direction != 'fwd':
           pairwise = []
           for node_i in range(self.len_unpadded()):
               pairwise.append(list(reversed(self.reversed().compute_log_conditionals_one(
                   self.len_unpadded()-1-node_i, probabilistic_masks))))
       ...
       if direction is None:
           ...
           if direction != "split": raise ValueError(
               f"unknown direction argument '{direction}'")
   ...
   def compute_log_conditionals_one(self, condition_on, probabilistic_masks=True):
       ...
       # nodes are in topological order so we can simply loop in order
       for node_i in range(self.len_unpadded()):
           node = self.nodes[node_i]
           for next_node in node.nodes_next:
               if probabilistic_masks:
                   next_log_prob = self.nodes[next_node].fwd_log_prob
                   next_cond_prob = math.exp(
                       cond_log_probs[next_node]) + math.exp(next_log_prob) * math.exp(cond_log_probs[node_i])
                   cond_log_probs[next_node] = math.log(
                       next_cond_prob) if next_cond_prob > 0.0 else LOG_ZERO
               else:
                   next_log_prob = 0.0
                   next_cond_prob = max(math.exp(cond_log_probs[next_node]), math.exp(
                       next_log_prob) * math.exp(cond_log_probs[node_i]))
                   cond_log_probs[next_node] = math.log(
                       next_cond_prob) if next_cond_prob > 0.0 else LOG_ZERO
...
if __name__ == "__main__":
   """
   plf = "uttid-1 ((('<s>', 0, 1),),(('YES', 0, 1),),(('I\\'D', 0, 1),),(('LIKE', 0, 1),),(('TO', 0, 1),),(('FIND', 0, 1),),(('A', 0, 1),),(('FLIGHT', 0, 1),),(('FROM', 0, 1),),(('MEMPHIS', 0, 1),),(('TO', 0, 1),),(('ALMA', 0, 1),),(('<UNK>', 0, 1),),(('PUMPING', -1.81475937, 1),('</s>', -0.177784383, 4),),(('AND', 0, 1),),(('<UNK>', 0, 1),),(('</s>', 0, 1),),)"
   """
   parser = argparse.ArgumentParser()
   parser.add_argument("lat_plf_file")
   parser.add_argument("output_dir")
   args = parser.parse_args()

   with open(args.lat_plf_file) as f:
       for i, line in enumerate(tqdm(f)):
            line = line.strip()
            tokens = line.split('\t')
            uttid = tokens.pop(0).rstrip()
            lat_reader = LatticeReader()
            lattice = lat_reader.read_sent(tokens[0], i)
            output = os.path.join(args.output_dir, uttid)
            lattice.plot(output, show_log_probs=["fwd_log_prob", "bwd_log_prob"])
