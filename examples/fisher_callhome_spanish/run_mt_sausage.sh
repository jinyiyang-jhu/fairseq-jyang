...
acwt=0.1
lmwt=1.0
lat_conf=conf/lat_transformer_bpe_Nov18.sh
lat_trans_mdl_dir="exp/mt_sau_Dec13"
fairseq_bin_lat=$lat_trans_mdl_dir/bpe_bin
plf_dir="data/plf_sau_pruned_beam_${beam}_acwt_${acwt}_lmwt_${lmwt}_depth_thres_${lat_depth_thres}"
...
if [ $stage -le 3 ]; then
  ...
  local/lattice_preprocess/run_sausage_preprocess.sh --stage $sau_process_stage $fairseq_bin_lat || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  fairseq-preprocess --source-lang $src --target-lang $tgt \
      ...
      --testpref $tgt_dir/bpe/fisher_dev,$tgt_dir/bpe/fisher_dev2,$tgt_dir/bpe/fisher_test,$tgt_dir/bpe/callhome_devtest,$tgt_dir/bpe/callhome_evltest \
