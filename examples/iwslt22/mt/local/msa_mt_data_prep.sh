...
for name in "${sets[@]}"; do
    ...
    echo ${out_dir}/${name}.${src}-${tgt}.${src}.${src_case}
    echo ${out_dir}/${name}.${src}-${tgt}.${tgt}.${tgt_case}.tok 
    echo ${out_dir}/${name}.${src}-${tgt}.${tgt}.all.${tgt_case}
    
    python local/process_empty_lines.py \
        ${out_dir}/${name}.${src}-${tgt}.${src}.${src_case} \
        ${out_dir}/${name}.${src}-${tgt}.${tgt}.${tgt_case}.tok \
        ${out_dir}/${name}.${src}-${tgt}.${src}.all.${src_case} \
        ${out_dir}/${name}.${src}-${tgt}.${tgt}.all.${tgt_case} 

    num_src=$(wc -l "${out_dir}/${name}.${src}-${tgt}.${src}.all.${src_case}" | cut -d " " -f1)
    num_tgt=$(wc -l "${out_dir}/${name}.${src}-${tgt}.${tgt}.all.${tgt_case}" | cut -d " " -f1)
    ...
    for lan in ${src} ${tgt}; do
        ...
        train_size=$(python -c 'import sys; print(round(float(sys.argv[1]) * 0.998))' "${num_src}")
        dev_size=$(python -c 'import sys; print(round(float(sys.argv[1]) * 0.001))' "${num_src}")
        awk -v var="$train_size" 'NR<=var' \
            ${out_dir}/${name}.${src}-${tgt}.${lan}.all.${lan_case} \
...
for lan in ${src} ${tgt}; do
    ...
    cat ${out_dir}/*.${src}-${tgt}.${lan}.train.${lan_case} \
        > ${out_dir}/train.${src}-${tgt}.${lan}.${lan_case}
     
    cat ${out_dir}/*.${src}-${tgt}.${lan}.dev.${lan_case} \
        > ${out_dir}/dev.${src}-${tgt}.${lan}.${lan_case}
    
    cat ${out_dir}/*.${src}-${tgt}.${lan}.test.${lan_case} \
        > ${out_dir}/test.${src}-${tgt}.${lan}.${lan_case}
done
echo "$(date) All the sets are prepared !"