...
trainset="train"
num_bpe=2000
hyp_lan_bpename="_ta_from_ta-en_spm2000"
tgt_lan_bpename=""
...
bindir="exp_ta-en_ta_translated_en_true_spm2000_ta_from_ta2en_en_from_msa2en/bin_${hyp_lan}2${tgt_lan}"
...
hyp_lan_bpe_mdl="data/ta-en_clean/spm2000/${hyp_lan}_bpe_spm2000/bpe.model"
tgt_lan_bpe_mdl="data/msa-en_processed/spm2000/${tgt_lan}_bpe_spm2000/bpe.model"
bpedir=${datadir}/spm${hyp_lan_bpename}
...
if [ $stage -le 0 ]; then
    ...
    (
        ...
        bpe="${outdir}/train.bpe${hyp_lan_bpename}.${hyp_lan}-${tgt_lan}.${hyp_lan}"
    ...
    if [ -f "${bpedir}/${trainset}.bpe.${hyp_lan}-${tgt_lan}.${hyp_lan}" ]; then
        rm  "${bpedir}/${trainset}.bpe.${hyp_lan}-${tgt_lan}.${hyp_lan}"
    fi
    ...
    for n in $(seq 8); do
        ...
        cat "${outdir}/train.bpe${hyp_lan_bpename}.${hyp_lan}-${tgt_lan}.${hyp_lan}" \
            >> "${bpedir}/${trainset}.bpe.${hyp_lan}-${tgt_lan}.${hyp_lan}"
    done
fi
...
if [ $stage -le 1 ]; then
    ...
    for s in "dev" "test1"; do
        for lan in ${hyp_lan} ${tgt_lan}; do
            ...
            bpe="${bpedir}/${s}.bpe.${hyp_lan}-${tgt_lan}.${lan}"
...
if [ $stage -le 2 ]; then
    ...
    trainpref="${bpedir}/${trainset}.bpe.${hyp_lan}-${tgt_lan}"
    validpref="${bpedir}/dev.bpe.${hyp_lan}-${tgt_lan}"
    testpref="${bpedir}/test1.bpe.${hyp_lan}-${tgt_lan}"
