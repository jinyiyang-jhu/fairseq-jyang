...
bpe_dir=/home/ec2-user/workspace/data/1month/bpe_transcript_vs_lattice/bpe
...
for d in "${dropouts[@]}"; do
 ...
 for n_layer in "${num_layers[@]}"; do
   ...
   for lay_w in "${layer_widths[@]}"; do
     encoder_ffn_embed_dim=$lay_w
     decoder_ffn_embed_dim=$lay_w
     params="$dropout $encoder_layers $decoder_layers $encoder_ffn_embed_dim $decoder_ffn_embed_dim"
     grids[$idx]=$params
     idx=$((idx+1))
   done
 done
