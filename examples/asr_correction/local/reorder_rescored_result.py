...
def main():
 ...
 parser.add_argument('output_hyp_score', help='Output file for positional hyp scores]')
 args = parser.parse_args()
 
 fp_src = open(args.output_src, 'w', encoding='utf-8')
 fp_hyp = open(args.output_hyp, 'w', encoding='utf-8')
 fp_hyp_scores = open(args.output_hyp_score, 'w', encoding='utf-8')
 
 dict_src = {}
 dict_hyp = {}
 dict_hyp_scores = {}
 
 patterns = {'^S-':dict_src, '^P-':dict_hyp_scores}
 ...
 with open(args.utt_list, 'r', encoding='utf-8') as fp_utt_list:
   for line in fp_utt_list:
     ...
     if idx in dict_hyp_scores.keys():
       print(line + ', ' + dict_hyp_scores[idx], file=fp_hyp_scores)
     else:
       print('utterances {}: P-{} is missing in {}'.format(line, idx, args.results), file=sys.stderr)
     idx +=1
