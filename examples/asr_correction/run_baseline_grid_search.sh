...
bpe_dir=/storage/home/ec2-user/workspace/data/1month/bpe_transcript_vs_1best/bpe
...
encoder_ffn_embed_dims=(1024 2048)
heads=(4 8)
num_layers=(2 3)
...
declare -A grids
for a in "${heads[@]}"; do
 encoder_attention_heads=$a
 decoder_attention_heads=$a
 for b in "${num_layers[@]}"; do
   encoder_layers=$b
   decoder_layers=$b
   for c in "${encoder_ffn_embed_dims[@]}"; do
     encoder_ffn_embed_dim=$c
     decoder_ffn_embed_dim=$c
     params="$encoder_attention_heads $decoder_attention_heads $encoder_layers $decoder_layers $encoder_ffn_embed_dim $decoder_ffn_embed_dim"
     grids[$idx]=$params
     idx=$((idx+1))
   done
 done
...
( params=(${grids[$cuda_index]})
 encoder_attention_heads=${params[0]}
 decoder_attention_heads=${params[1]}
 encoder_layers=${params[2]}
 decoder_layers=${params[3]}
 encoder_ffn_embed_dim=${params[4]}
 decoder_ffn_embed_dim=${params[5]}
