...
ref_name="transcript" # transcript or asr_oracle
sets=("train" "dev" "test")
valid_sets=("${sets[@]:1}")
...
if [ $stage -le 0 ]; then
 mkdir -p $bpe_dir/corpus || exit 1
 echo "`date` => Getting BPE vocab from training data"
 # Get BPE vocab from training data
 ref_train=$data_dir/train/transcript/transcript.txt
 hyp_train=$data_dir/train/asr_1best/asr_1best.txt
 bpe_text=$bpe_dir/corpus/train.txt
 cat $ref_train $hyp_train > $bpe_text
 cat $data_dir/train/transcript/selected_utt.index $data_dir/train/asr_1best/selected_utt.index \
   > $bpe_dir/corpus/selected_utt.index
 subword-nmt learn-joint-bpe-and-vocab --input $bpe_text -s 10000 \
   -o $bpe_dir/code_file.txt \
   --write-vocabulary $bpe_dir/vocab.all.txt || exit 1
fi
